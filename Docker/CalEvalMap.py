...
from cardiac_utils_test import determine_aha_coordinate_system, determine_aha_part, remove_small_cc, get_largest_cc, determine_aha_segment_id
import pandas as pd
import h5py
import os
import copy
        ...
        for y in range(0, zoom.shape[0]):
            for x in range(0, zoom.shape[1]):
                ...
                if S[1] < 0:
                    ...
                    s = " "  # 试验最小二乘法函数leastsq得调用几次error函数才能找到使得均方误差之和最小的k、b
                    p0 = [100, 2]
...
def CalSaveT2map(T2Recon, invtime_path, target_dir, folder):
    if os.path.isfile(target_dir +'/'+ folder + '/CalT2Reconmap.mat'):
        ...
        data = np.genfromtxt(invtime_path, delimiter=',')  # 读取.csv文件
...
#
#     for nsignflip in range(6):  # 优化循环范围为6，因为后续对称的情况可以在前面的基础上反转得到
#         yf[:nsignflip+1] = -y[:nsignflip+1]
#
#         pool = Pool()  # 创建进程池
#         func_args = [(func_orig, inversiontime, yf, p0_initial)] * 9
#         results = pool.map(fit_curve, func_args)  # 并行计算拟合曲线
#
...
def CalSaveT1map(T1Recon, invtime_path, target_dir, folder):
    ...
    print("总运行时间: {:.2f}秒".format(elapsed_time))
    return T1ReconMap
